...
const dataSets = [
  {
    productName: "Q5 Chocolate (12x20)",
    pallets: "3 Yellow",
    perPalletUnitQty: 22,
    order: "Promo",
    price: "€43",
...
const filterableColumns = ["order", "customer"]; // you requested this

export default function ManufacturerOrderDetailsData() {
  ...
  const uniqueFilterValues = useMemo(
    () => ({
      order: [...new Set(rawData.map((r) => r.order).filter(Boolean))],
      customer: [...new Set(rawData.map((r) => r.customer).filter(Boolean))],
    }),
    [rawData]
  );
  ...
  const columns = useMemo(
    () => [
      ...
      { field: "perPalletUnitQty", header: "Per Pallet Unit Quantity" },
      ...
      { field: "totalPrice", header: "Total Price" },
      {
        ...
        header: "Customer",
        body: (rowData) => (
          <button className="w-[110px] h-[28px] justify-center flex items-center gap-1 px-3 py-1 bg-[#EA7D00] hover:bg-[#CC6600] text-white text-[12px] rounded dark:text-[#0D0D0D] dark:bg-[#EA7D00]">
            {rowData.customer}
          </button>
        ),
      },
