...
const tabTitles = [
  {
    heading: "Reports",
    subheading:
      "Gain deep insights into sales, inventory, and logistics reports.",
  },
];

const dataSets = [
  {
    reportName: "Sales Report 1",
    dateCreated: "12/01/2025",
    reportType: "Sales",
    period: "12/01/2025 - 12/01/2025",
    action: "View Report",
  },
  {
    reportName: "Sales Report 1",
...
export default function ReportData() {
  ...
  const uniqueFilterValues = useMemo(
    () => ({
      reportType: [
        ...new Set(rawData.map((r) => r.reportType).filter(Boolean)),
      ],
    }),
    [rawData]
  );
  ...
  const columns = useMemo(
    () => [
      ...
      { field: "period", header: "Period" },
      {
        ...
        body: (rowData) => (
          <button
            className="justify-center w-[90px] h-[28px] flex items-center gap-1 px-3 py-1 bg-white text-[#EA7D00] text-[12px] rounded dark:bg-black dark:text-[#EA7D00] border border-[#EA7D00]  dark:border-[#EA7D00]"
      ...
      },
      {
        header: "",
        body: (rowData) => (
          <button className="w-[90px] h-[28px] flex items-center gap-1 px-3 py-1 bg-[#EA7D00] hover:bg-[#CC6600] text-white text-[12px] rounded dark:text-[#0D0D0D] dark:bg-[#EA7D00] justify-center">
            Download
          </button>
        ),
      },
  ...
  const filteredData = useMemo(() => {
    return rawData.filter((row) => {
      ...
      const passesSearch = globalFilter
        ? ["reportName", "dateCreated", "reportType", "period"].some((k) =>
            row[k]
              ?.toString()
              .toLowerCase()
            ...
            <div className="w-full">
              ...
              <div className="flex flex-col gap-2">
                {Array.from({ length: rowsPerPage }).map((_, rowIdx) => (
                  <div
                    key={rowIdx}
                    className="flex flex-row gap-2 items-center w-full justify-between"
                  >
