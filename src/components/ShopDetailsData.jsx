...
export default function ShopDetailsData() {
  ...
  const ShopDetailTable = () => {
    ...
    const uniqueFilterValues = useMemo(
      () => ({
        product: [...new Set(rawData.map((r) => r.product).filter(Boolean))],
    ...
    const columns = useMemo(() => [
      ...
      },
      {
        ...
        header: "Action",
        body: (rowData) => (
          <button className="w-[120px] h-[28px] gap-2 flex items-center justify-center text-center bg-[#EA7D00] text-white dark:text-black text-[12px] rounded">
