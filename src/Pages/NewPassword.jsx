...
export default function NewPassword() {
  ...
  // read ?tab=signup or ?tab=login from URL
  const searchParams = new URLSearchParams(location.search);
  const initialTab = searchParams.get("tab") || "login"; // default to login

  const [activeTab, setActiveTab] = useState(initialTab);

  // keep tab updated when URL changes
  useEffect(() => {
    const newTab = new URLSearchParams(location.search).get("tab") || "login";
    setActiveTab(newTab);
  }, [location.search]);

  // helper: change tab & update URL
  ...
  // states for login (now only password & confirmPassword)
  ...
  const handleLoginSubmit = () => console.log("Login:", loginData);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* --- Login Section (password + confirmPassword only) --- */}
        {activeTab === "login" && (
          <>
            ...
            <div className="flex pt-3 w-full mb-2">
              <ActionButton
                label="Login"
                labelClass="font-normal text-[12px] lg:text[16px]"
                buttonClass="text-[16px] h-[45px] w-full bg-[#EA7D00] text-white focus:outline-none focus:ring-0 hover:bg-[#cc6900] rounded-lg"
